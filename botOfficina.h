...
const unsigned long intervalloGiorno = 24 * 60 * 60 * 1000;
unsigned long controlloGiorno = 0;
...
#define BOT_TOKEN "5965326951:AAFKBZBQJj8DaGoh2MxUpCEC-5k0tba4ZHo"
...
#define EAP_PASSWORD "DaiCoraggioTeLoAspettavi" //password for eduroam account
...
void readArrayFromFile(){
  ...
  int index = 0;
  while ((file.available()) && (index < Lunghezza) ) 
  {
    String value = file.readStringUntil('\n');
    Serial.println(value);
    lista_persone[index] = value.toInt();
    index++;
  }
...
void writeArrayToFile() {
  ...
  if (!file) {
    ...
    bot.sendMessage(String(744915921) , "Failed to open file for writing" , "Markdown");
    return;
  }
  file.print("");
  for (int i = 0; i < Lunghezza; i++) {
    file.println(lista_persone[i]);
  }
  file.close();
}
  ...
  for (int i = 0; i < numNewMessages; i++)
  {
    ...
    if(text == "/nerd")
    {
      ...
      for (int i = 0; i < Lunghezza; i++) 
      {
        if(lista_persone[i]==0) break;
        Messaggio += "\n";
        Messaggio += String(lista_persone[i]); 
      }
...
void loop() {
  ...
  if (currentMillis - controlloGiorno >=intervalloGiorno) 
  {
    numMessaggi=0;
    controlloGiorno = currentMillis;
  }
